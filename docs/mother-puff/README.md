...
## The arch behind the boss

`dist/assets/mother-puff-arch.glb` is the supplied `porous+tree+3d+model.glb` (Tripo; one mesh, 11,052 triangles, a single 2048×2048 colour map, no normal or roughness maps) repacked with the map at 1024×1024, like the other four blighted sculptures; geometry and UVs are untouched, and `mother-puff-arch.json` carries the source and shipped checksums, byte counts and texture sizes that `tests/mother-puff-assets.mjs` checks. It stands directly behind Mother Puff as the one tree her silhouette reads against. Its stone was sculpted paler than the blight, so the shader brings it down to charcoal by how little colour a texel carries, which leaves the moss dripping over it green. Healing swaps it for a healthy canopy like the rest.
