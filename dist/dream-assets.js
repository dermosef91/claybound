...
// abstract sculpture whose hung pieces ARE that canopy, the Melted Parade's
// clay hat — stacked five high on the hat-worm's plinth and three high on
// every hatworm's head — the caterpillar that is the hatworm's body, and the
// parade's giraffe. Loaded once per World, kept across level rebuilds, cloned
// per placement — the same shape as the forest's and the canyon's sets; the
// two creatures are skinned, so they clone through SkeletonUtils. The
// collision never comes from here: a dome is still the arc in simulation.js,
// a saucer is still its deck's flat top, the canopy and the hats are scenery,
// the hatworm is still HATWORM in dream-enemy-rules.js and the giraffe's decks
// are still the level's ledges; these only replace what is seen.
export const DREAM_FILES={mint:'dream-planet-mint.glb',raspberry:'dream-planet-raspberry.glb',saucerMint:'dream-saucer-mint.glb',saucerRaspberry:'dream-saucer-raspberry.glb',hat:'dream-hat.glb',sculpture:'dream-sculpture.glb',caterpillar:'dream-caterpillar.glb',giraffe:'dream-giraffe.glb'};
