...
// take dt as they always did. With it on, nothing moves until an eighth of a
...
// worth, which may be a little over the eighth when frames landed unevenly.
...
// This is what such a pose should read this frame: with the clock on, fresh
// from the simulation whenever the pose advances and otherwise what it read at
// the last exposure; with it off, always the live value — a paused frame is
// not a hold.
export function heldSample(clock,view,step,read){
  if(!clock?.on||step>0||!view.held)view.held=read();
  return view.held;
}
...
// back where the authored offset left it once the setting is off.
export const BOIL=.006;
const hash=(n,k)=>{const x=Math.sin(n*12.9898+k*78.233)*43758.5453;return x-Math.floor(x);};
const seed=name=>{let h=0;for(const ch of name||'')h=(h*31+ch.charCodeAt(0))%1000;return h;};
export function boilPuppet(root,clock){
  ...
  root.traverse(o=>{
    ...
    for(const m of Array.isArray(o.material)?o.material:[o.material]){
      ...
      const n=clock.frame+seed(m.name);
