...
function bridgeMaterials(w){
  ...
  for(let y=0;y<height;y++)for(let x=0;x<width;x++){
    ...
    let fibre=v*9+.33*Math.sin(u*15+v*4)+.13*Math.sin(u*41-v*8);
    ...
    const tone=.84+.055*Math.sin(fibre*2.1)-vein*.14-fine*.035+.025*Math.sin(x*1.73+y*3.17);
...
export function makeRopeBridge(w,s,g){
  ...
  for(let i=0;i<3;i++){
    ...
    for(let j=0;j<=steps;j++){
      const x=s.w*j/steps,y=deck(x)+bias-.085+Math.sin(j*.3+i)*.005;
      if(j)shape.lineTo(x,y);else shape.moveTo(x,y);
    }
    for(let j=steps;j>=0;j--){const x=s.w*j/steps;shape.lineTo(x,deck(x)+bias-.27+Math.sin(x*7+i)*.012);}
    shape.closePath();
    const geometry=new THREE.ExtrudeGeometry(shape,{depth:.53,bevelEnabled:true,bevelThickness:.07,bevelSize:.085,bevelSegments:5,steps:1});
  ...
  const left=-.38,right=s.w+.38,railHeight=x=>{
    const u=(x-left)/(right-left);return .69-4*(sag+.38)*u*(1-u);
  };
  for(const [index,x]of [left,right].entries()){
    ...
    const post=w.cylinder(.24,1.04,materials.wood,root,0,.48,0);
    // Turn the fibre direction up the post instead of around its circumference.
    const geometry=post.geometry.clone(),uv=geometry.attributes.uv,position=geometry.attributes.position;
    for(let j=0;j<uv.count;j++){
      const u=uv.getX(j),v=uv.getY(j),y=position.getY(j),taper=1+.045*Math.sin(y*5+index)-.018*Math.cos(u*Math.PI*6);
      uv.setXY(j,v*.56,u);position.setXYZ(j,position.getX(j)*taper,y,position.getZ(j)*taper);
    }
    geometry.computeVertexNormals();
    post.geometry=geometry;
    w.ball(.235,.075,.225,materials.endgrain,root,0,.99,0);
    const wrap=[];for(let j=0;j<=90;j++){const t=j/90,a=t*Math.PI*6;wrap.push([x+Math.cos(a)*.27,.58+t*.19,.76+Math.sin(a)*.27]);}
    braid(wrap,.065);
    braid([[x+(index?-.24:.24),.75,.95],[x+(index?-.30:.30),.60,1.02],[x+(index?-.16:.16),.54,.97]],.066);
  }
  ...
  for(let i=0;i<ties;i++){
    const x=s.w*(.1+.8*i/(ties-1)),y=deck(x),top=railHeight(x),lean=((ties-1)/2-i)*.05;
    ...
    for(const dx of [-.058,.058]){
      braid([[x+dx-lean,y+.07,1.02],[x+dx-lean*2,y-.30,1.06],[x+dx,y-.38,.82],[x+dx,y-.35,-.82],[x+dx,y-.21,-1],[x+dx,y+.09,-.83],[x+dx+lean,y+.09,.76],[x+dx-lean,y+.07,1.02]],.060);
