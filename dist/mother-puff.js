...
export function createMotherPuff(w,b){
  ...
  if(w.forestAssets){
    for(const [dx,z,width,dy]of [[-21,-16,16,-5],[21,-17,17,-5],[-9,-28,19,-7],[10,-31,21,-8]])
      forestModel(w,'canopy',root,b.x+dx,b.y+dy,z,width,dx<0?.18:-.15,true);
    for(const dx of [-13,13]){
      forestModel(w,'heroMushroom',root,b.x+dx,b.y,-7,5.5,dx*.03,true);
      forestBloom(w,root,b.x+dx,b.y-.12,-3.6,3.2,dx*.02);
    }
  }
  ...
  for(const [i,dx]of [-12,-9.3,-7.4,7.4,9.5,12.4].entries()){
    ...
    w.ball(.28*k,.48*k,.28*k,'cream',root,x,y+.4*k,z);
    w.ball(.67*k,.33*k,.52*k,mats.purple,root,x,y+.87*k,z);
    w.ball(.12*k,.04*k,.12*k,'cream',root,x-.2*k,y+1.17*k,z+.1);
  }
  const gates=[];
  for(const x of [b.left,b.right]){
    const g=new THREE.Group();g.position.set(x,b.y,0);g.name='Living root curtain';root.add(g);gates.push(g);
    for(let i=0;i<4;i++){
      const points=Array.from({length:9},(_,j)=>new THREE.Vector3(Math.sin(j*.64+i)*.28,j*1.6,-1.3+i*.72));
      w.mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points),24,.14+i*.018,6,false),'bark',g);
      for(let j=1;j<4;j++)w.ball(.37,.16,.2,'top',g,Math.sin(j+i)*.3,j*2.6,-1.3+i*.72);
    }
  }
  const crown=new THREE.Group();crown.name='Stomp target petals';crown.position.set(b.x,b.y+MOTHER_PUFF.height+.06,.15);root.add(crown);
  for(let i=0;i<6;i++){const a=i*Math.PI/3;const m=w.ball(.32,.09,.21,mats.orange,crown,Math.cos(a)*.67,0,Math.sin(a)*.5);m.rotation.y=-a;}
  return {root,pose,models,gates,crown,effects:new Map()};
}
...
export function animateMotherPuff(w,game){
  ...
  const t=game.time,quiet=w.reducedMotion,inhale=b.state==='inhale',hurt=b.state==='hurt',defeated=b.state==='defeated';
  const breath=quiet?0:inhale?Math.min(1,b.stateTime/MOTHER_PUFF.inhale)*.1:Math.sin(t*1.55)*.013;
  v.pose.scale.set(1+breath,1-breath*.35,1+breath);
  v.pose.rotation.z=hurt&&!quiet?Math.sin(b.stateTime*23)*.045*Math.exp(-b.stateTime*2):0;
  if(defeated){const q=Math.min(1,b.stateTime/1.2);v.pose.scale.set(1+q*.09,1-q*.16,1+q*.09);}
  ...
  for(const g of v.gates)g.visible=sealed;
  v.crown.visible=sealed&&!hurt&&!!game.player.motherBounce;v.crown.scale.setScalar(1.22);
...
export function motherCamera(b,p,viewW,viewH,landscape){
  ...
  const x=landscape?Math.max(b.x-5,Math.min(b.x+3,p.x+3)):p.x+Math.max(-5,Math.min(5,(b.x-p.x)*.6));
  return {x,y:b.y+(landscape?5.7:6.5)+Math.max(0,p.y-b.y-7)*.2};
}
