...
export default chapter({
  ...
  platforms:[
    ...
    p('sand-catch',100,17,6.9,'ledge',{recovery:true}),p('sand-return',117,4,8.8,'ledge',{recovery:true}),
    ...
    p('arch-return',159,3.4,16.7,'ledge'),p('arch-roof',170,6,16),p('arch-drop',178,4,13.8,'ledge'),
    p('last-rest',184,7,12,'stone',{checkpoint:187,landmark:'oasis',rest:true}),p('last-well',191,6,12,'stone',{landmark:'windmill'}),
    p('valve2',194,1.8,12.13,'switch',{channel:'wind-b',latch:true}),p('sky1',199,4,13.5,'ledge'),
    ...
    p('well-flower',50,3.2,13.5,'ledge',{optional:true}),p('basin-flower',109,3.2,5.1,'ledge',{optional:true}),p('arch-flower',162.5,3.2,18.3,'ledge',{optional:true})
  ],
  route:['start','lift1','arrival','notch','rope-cross','lookout',['windwell','walk'],['valve1','walk'],'wind-step','wind-crown','wind-exit','downstep','wind-gondola','oasis',['basin','walk'],'sand1','sand2','sand3','sand-rest','sand4','sand5','arch-entry','arch-shelf','arch-lift','arch-balcony','arch-roof','arch-drop','last-rest',['last-well','walk'],['valve2','walk'],'sky1','sky-lift','sky2','sky-rest','sky-sand','sky-rope','bell-roof'],
  detours:[path(['wind-crown','wind-turn','well-flower','wind-turn','wind-crown','wind-exit']),path(['sand2',['sand-catch','fall'],['basin-flower','fall'],'sand-catch','sand-return','sand-rest']),path(['arch-balcony','arch-return','arch-flower','arch-roof'])],
  recoveries:[path(['sand-catch','sand-return','sand-rest'])],
  ...
  circuits:[{source:'valve1',channel:'wind-a',targets:['wind-step','wind-crown'],kind:'wind'},{source:'valve2',channel:'wind-b',targets:['sky1','sky2'],kind:'wind'}],
  coins:[...row(4,1,3),...arc(9,1,10,2,5,1.7),...row(27,4.3,2),...row(44,5.3,2),...arc(53,6.6,5,4.7,5,.5),...row(71,11,2),...row(84,10.2,3),...row(101,10.7,2),...row(107,11.3,2),...row(113,11.1,2),...row(126,12.4,2),...row(144,10.4,3),...arc(159,13,6,3.1,5,1),...row(185,13,3),...arc(200,15,13,5.5,7,1),...row(219,21,2),...row(239,23,5)],
  stamps:[{x:51.6,y:14.5},{x:110.6,y:6.1},{x:164,y:19.3}],
  ...
  hints:[{x:0,end:9,title:'Find your stride',text:'Move with A / D or arrows. Hold jump to leap farther.'},{x:43,end:51,title:'Wake the windwell',text:'Step on the cream valve. The rising ribbons lift your jumps.'},{x:91,end:100,title:'Light on the sand',text:'Pale ledges sink after landing. Keep your rhythm; lower shelves catch a slip.'}],
