...
export function buildForestBackdrop(w){
  const far=layer(w,'Skybridge Falls skyline',.18),cascade=layer(w,'Cascade terraces',.24),sky=layer(w,'Wildwood clouds',.09),middle=layer(w,'Hazy canopy bridges',.38),near=layer(w,'Breathing forest trunks',.62);
  ...
  // The supplied cascade is the one landmark the whole chapter can be read
  // against, so it repeats on a wider interval than the canopy bands.
  for(let i=-2;i<6;i++)forestModel(w,'waterfall',cascade,i*43+14,-6.9-(i%2)*.8,-38,12+(i%3)*1.4,-.05+(i%2)*.1,true);
