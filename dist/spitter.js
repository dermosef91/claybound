...
export function createSpitterView(w,e){
  ...
  const view={kind:'spitter',loaded:true,root,body,character,model,clip,tracks,bones,walkPhase:0,
    ...
    deathTime:0,reducedMotion:!!w.reducedMotion};
...
export function animateSpitter(v,e,dt,status){
  ...
  const charge=e.aiState==='charge'?smooth(e.stateTime/SPITTER.charge):0;
  ...
  if(v.head){
    const breath=v.reducedMotion||!['watch','patrol'].includes(e.aiState)?0:Math.sin(v.time*1.7+e.id*.61)*.009*(1-charge);
