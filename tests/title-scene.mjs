...
assert.equal(title.flagCloths.length,1,'the supplied flag cloth receives wind morphs');
const cloth=title.flagCloths[0],geometry=cloth.geometry,base=geometry.attributes.position;
let moving=0;
for(let i=0;i<base.count;i++){
  const moves=geometry.morphAttributes.position.some(a=>a.getY(i)!==0||a.getZ(i)!==0);
  if(moves){
    moving++;assert(base.getX(i)>.012&&base.getY(i)>.24&&base.getZ(i)<-.32,'only the cloth beyond the pole moves');
  }
  for(const attr of geometry.morphAttributes.position){
    assert.equal(attr.getX(i),0,'the flag stays attached horizontally');
    assert(Math.abs(attr.getY(i))<=.0081&&Math.abs(attr.getZ(i))<=.0221,'wind remains subtle');
  }
}
assert(moving>100&&moving<200,'wind is confined to the flag, leaving the mesa and cactus still');
for(const attr of geometry.morphAttributes.normal)assert(attr.array.every(Number.isFinite));
const sourceCloth=w.titleMesa.scene.getObjectByName(cloth.name);
...
const flagPose=[...cloth.morphTargetInfluences];for(let i=0;i<60;i++)title.update(1/60);
assert.notDeepEqual(cloth.morphTargetInfluences,flagPose,'wind travels through the cloth over time');
assert(sourceCloth.morphTargetInfluences.every(v=>v===0),'title animation does not change the cached source pose');
w.reducedMotion=true;const t=title.time,frozenFlag=[...cloth.morphTargetInfluences];title.update(.5);assert.equal(title.time,t);
assert.deepEqual(cloth.morphTargetInfluences,frozenFlag,'reduced motion freezes the flag');w.reducedMotion=false;
...
console.log('PASS live title: responsive hero composition, source textures, skeletal idle, pinned flag wind, reduced motion, single hero ownership and independent scene caches');
