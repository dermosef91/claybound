...
// --- the level itself -------------------------------------------------------
{
  ...
  assert.deepEqual(lab.shaping.map(s=>s.id),['sag','catapult','stamp','form','lump','dig','lintel','mould','wet','marble','fix']);
  assert.equal(lab.layoutVersion,3,'the bench grew a station, so saves of the old bench are not resumed');
