...
view.root.updateMatrixWorld(true);const swollen=new THREE.Box3().setFromObject(view.growth.bands[0].root,true);assert(swollen.max.y>b.y+6.4&&swollen.max.y<b.y+7.6,'separate plugs sit close to the original cap');assert(view.growth.bands.every(x=>!x.sack),'no balloon-shaped cap is added');assert(view.environment.corruption.filter(p=>p.side==='right'&&!/^semi-/.test(p.kind)).every(p=>p.amount===1),'stone on the boss\'s side is wholly hers');assert(view.environment.corruption.filter(p=>/^semi-/.test(p.kind)).every(p=>p.amount>0&&p.amount<.6),'half-turned sculptures keep their healthy side');assert(view.environment.corruption.some(p=>p.side==='left'&&p.amount>.3&&p.amount<.8),'the middle is where things are caught hardening');assert(view.environment.corruption.some(p=>p.side==='left'&&p.amount<.2),'and the approach stays healthy');assert(view.environment.porous.every(p=>p.poreCount>0));
