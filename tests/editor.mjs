...
// The incognito export is the source of truth, including deliberate deletions.
{
 ...
 assert.equal(canonical.layoutVersion,'editor-12-8ur6hp','canonical canyon matches the approved editor export');
 ...
 for(const key of ['rule','free','relax','shaped','pace','give','cueX','gesture','clump','solution'])assert.deepEqual(pocket[key],authored[key],`the canonical canyon keeps its formable mass's ${key}`);
 ...
 // `give` — the sag springs riding the mass — is `true` for the bench's numbers
 // or the numbers to change, in the table's own order; anything else is refused.
 {const draft=structuredClone(LEVELS[0]);draft.shaping.find(s=>s.id==='canyon-pocket').give=true;
  assert.equal(validateDraft(draft,LEVELS[0]).shaping.find(s=>s.id==='canyon-pocket').give,true,'a station that gives with the bench\'s numbers says so with true');}
 {const draft=structuredClone(LEVELS[0]);draft.shaping.find(s=>s.id==='canyon-pocket').give={tension:30,weight:400};
  assert.deepEqual(JSON.stringify(validateDraft(draft,LEVELS[0]).shaping.find(s=>s.id==='canyon-pocket').give),'{"weight":400,"tension":30}','a give\'s overrides round-trip in the table\'s order');}
 for(const bad of [{rule:'sag'},{gesture:'sideways'},{clump:[[2,0]]},{clump:[]},{solution:[{x:130,lift:0,dx:0,dy:0,t:0}]},{shaped:0},{solution:Array.from({length:41},()=>({x:130,lift:0,dx:1,dy:1,t:1}))},{give:'yes'},{give:[400]},{give:{weight:-1}},{give:{spacing:.5}},{give:{softness:2}}]){
...
// Decoration is edited like everything else and played like nothing at all.
{
 ...
 assert.deepEqual(s.level.decor,[{kind:'purple-arch',x:110,y:3.5,size:20,z:-16}],'the canyon hands its authored arch to the workshop as an ordinary placement');
