...
// The incognito export is the source of truth, including deliberate deletions.
{
 ...
 assert.equal(LEVELS[0].layoutVersion,7);assert(!LEVELS[0].custom);
 assert.equal(canonical.layoutVersion,'editor-7-1ctpvvr','canonical canyon matches the approved editor export');
}
{
 ...
 assert.equal(LEVELS[1].layoutVersion,9);assert(!LEVELS[1].custom);assert(!draft.boss,'old forest imports keep their original finish');
...
}
{
 ...
 assert.equal(LEVELS[2].layoutVersion,9);assert(!LEVELS[2].custom);
...
}
{
 ...
 assert.equal(LEVELS[3].layoutVersion,5);assert(!LEVELS[3].custom);
