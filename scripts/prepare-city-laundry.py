"""Repack the generated laundry nook's embedded textures; keep geometry and UVs intact.

Usage: python scripts/prepare-city-laundry.py PATH_TO_LAUNDRY_GLB
"""
...
(assets/'city-laundry.glb').write_bytes(result)
(assets/'city-laundry.json').write_text(json.dumps({'source':source.name,'sourceSha256':digest(data),'shippedSha256':digest(result),'sourceBytes':len(data),'shippedBytes':len(result),'textures':dimensions,'geometryBufferSha256':geometry,'geometryUnchanged':True},indent=2)+'\n')
print(f'Laundry: {len(data):,} → {len(result):,} bytes; geometry and three material maps retained')
