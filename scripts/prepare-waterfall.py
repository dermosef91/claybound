"""Repack the supplied Clay Waterfall's embedded textures; keep geometry and UVs intact.

Usage: python scripts/prepare-waterfall.py PATH_TO_WATERFALL_GLB
"""
...
for i,image in enumerate(doc['images']):
    ...
    output=BytesIO();im.save(output,format='JPEG',quality=94 if i in normal_images else 90,subsampling=0,optimize=True)
...
(assets/'forest-waterfall.glb').write_bytes(result)
(assets/'forest-waterfall.json').write_text(json.dumps({'source':source.name,'sourceSha256':digest(data),'shippedSha256':digest(result),'sourceBytes':len(data),'shippedBytes':len(result),'textures':dimensions,'geometryBufferSha256':geometry,'geometryUnchanged':True},indent=2)+'\n')
print(f'Waterfall: {len(data):,} -> {len(result):,} bytes; geometry and three material maps retained')
