"""Repack the supplied drifter's textures for mobile, preserving its sculpture.

Usage: python scripts/prepare-drifter.py PATH_TO_ORIGINAL_GLB
...
(root/'dist/assets/drifter.glb').write_bytes(result)
(root/'dist/assets/drifter.json').write_text(json.dumps(manifest,indent=2)+'\n')
print(f'Dust Drifter: {len(data):,} → {len(result):,} bytes; geometry preserved')
