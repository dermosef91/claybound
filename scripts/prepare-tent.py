"""Repack the supplied tent's embedded textures; keep geometry and UVs intact.

Usage: python scripts/prepare-tent.py PATH_TO_TENT_GLB
"""
...
(assets/'canyon-tent.glb').write_bytes(result)
(assets/'canyon-tent.json').write_text(json.dumps({'source':source.name,'sourceSha256':digest(data),'shippedSha256':digest(result),'sourceBytes':len(data),'shippedBytes':len(result),'textures':dimensions,'geometryBufferSha256':geometry,'geometryUnchanged':True},indent=2)+'\n')
print(f'Tent: {len(data):,} → {len(result):,} bytes; geometry and three material maps retained')
