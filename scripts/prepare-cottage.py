"""Repack the supplied cottage's textures for mobile, preserving its sculpture.

Usage: python scripts/prepare-cottage.py PATH_TO_ORIGINAL_GLB
...
(root/'dist/assets/cottage.glb').write_bytes(result)
(root/'dist/assets/cottage.json').write_text(json.dumps(manifest,indent=2)+'\n')
print(f'Cottage: {len(data):,} → {len(result):,} bytes; geometry preserved')
