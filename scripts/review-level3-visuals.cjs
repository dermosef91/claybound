...
const root=path.resolve(process.env.ROOT||PROJECT);
...
review(async({page,url,errors,requests:badRequests})=>{
 ...
 {
  await patchApp(page,{root,expose:'window.playtest={manual:false,get game(){return game},get world(){return world},get input(){return input},begin,home,draw(dt=0){world.render(game,dt);healthHUD.draw(game,dt);updateHUD(performance.now());},tick(n){for(let i=0;i<n;i++)game.tick(FIXED_DT,input);}};'});
  await page.goto(url+'/');
 ...
 page:{deviceScaleFactor:1},
 // Ember Caverns is judged on what the GPU actually draws, so this one asks
 // for the real Metal backend rather than accepting a software fallback.
 args:['--no-sandbox','--use-angle=metal','--enable-gpu-rasterization']
