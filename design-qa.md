...
## Loading-screen alignment — 2026-09-12

final result: passed

Both blocking world loading and chapter loading use the same `#loading` overlay. Its previous serif wordmark and flat brown backdrop differed from the title screen (P1; `docs/loading-design/before.png`). The overlay now reuses the exact `logo.webp` artwork and `clay-logo-orange` pigment filter, shared sky/cream tokens, Clay Sans supporting type, uppercase tracked tagline, and orange/cream clay progress treatment. The logo keeps its native 1600:456 ratio at every viewport. Existing status messages and loading behavior are retained.

Evidence: `docs/loading-design/title-loading-comparison.png` places the title screen and completed loader together, both originally 1280 × 720, displayed at equal density/scale. The loading composition intentionally centers the shared brand and progress feedback and does not require the 3D world to be loaded. Initial progress texture appeared gray and metallic (P2, `iteration-1.png`); corrected the material layering to preserve cream/orange pigment. Final startup and chapter captures are `desktop.png` and `chapter-desktop.png`; additional captures are `square.png` (900 × 900), `phone.png` (390 × 844), and `landscape-phone.png` (844 × 390), all under `docs/loading-design/`.

Fidelity review: title artwork, alpha, pigment filter, and proportions match the title screen. Supporting font, tracking, hierarchy and copy follow its branding. Spacing is centered with clear separation between brand and progress/status, and the progress bar uses the existing clay height texture. The simpler background is intentional for a screen shown before assets are ready. All content fits in the verified viewports; raw measurements are in `layout-verification.json`. Both indeterminate startup animation and determinate chapter feedback render correctly. Reduced-motion CSS disables nonessential animation.

Visual captures use a deterministic review page generated by `scripts/review-loading.mjs` from production `index.html` and production styles, holding transient loading states for inspection. No duplicate loading component is maintained. Runtime behavior was checked with the existing app/DOM integration suite: slow/shared loading, concurrent Play requests, world readiness, chapter loading/re-entry, dialog focus, editor transitions and saved-game handling all pass. No browser console errors were recorded. Diff whitespace check passed.

- [x] Shared clay logo and matching supporting typography.
- [x] Consistent startup and chapter loading design.
- [x] Desktop, square, portrait and short landscape verification.
- [x] Existing loading/transition regression checks pass.
